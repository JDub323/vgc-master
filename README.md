...
| [models/value_heads.py](models/value_heads.py) | exp/value-head: replacement leaf-value architectures (CLS MLP, attention pooling, dedicated fine-tuned value net) behind `ValueAugmentedNet`, which keeps the frozen baseline's policy/aux outputs bit-identical and swaps only the value scalar. |
| [value_labels.py](value_labels.py) | exp/value-head: sidecar end-of-game margin labels (faint/HP differentials) and per-row game-progression fractions, aligned 1:1 with the prepped shards, with recomputed weight/outcome/turn-bucket alignment proofs. |
| [value_lab.py](value_lab.py) | exp/value-head: trains the candidate value bricks against the frozen baseline (optionally down-weighting early-game rows by `--progression-floor`, since their z label reflects many turns of subsequent play), ranks them on the validation split with evaluate.py --value-style metrics, and packages the winner as a combined checkpoint for `agent_server.py --agent search-vh`. |
