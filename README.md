...
| [value_labels.py](value_labels.py) | exp/value-head: sidecar end-of-game margin labels (faint/HP differentials) aligned 1:1 with the prepped shards, with recomputed weight/outcome alignment proofs. |
| [value_lab.py](value_lab.py) | exp/value-head: trains the candidate value bricks against the frozen baseline, ranks them on the validation split with evaluate.py --value-style metrics, and packages the winner as a combined checkpoint for `agent_server.py --agent search-vh`. |
